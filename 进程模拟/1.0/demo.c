...
//用户输入指令，将指定PCB阻塞，即从运行/等待队列移动到阻塞队列中
void go_sleep(){
	...
	for(ite = plist.begin();ite!=plist.end();){
		if((*ite).Sleep == 1){
			...
			ite = plist.erase(ite);		//从运行/等待队列移除
			return ;
		}
		else ite++;		//！！！坑点：遍历移除方法
...
void stop(int mp){
	...
	if(m!= NULL){
	//	m->wait_time = 0;
		m->is_run = 0;		//is_run为1表示其正在运行状态，0表示等待状态
	}
}
void write_to_file(){
	char chr[100];
	...
	if((fp = fopen("info.txt","at+")) != NULL){
		...
		fputs(chr,fp);
		fputs("\n已运行时间： ",fp);
		sprintf(chr,"%d",plist.front().real_run_time);
		fputs(chr,fp);
		fputs("\n总运行时间： ",fp);
		sprintf(chr,"%d",plist.front().to_run_time);
		fputs(chr,fp);
		fputs("\n已等待时间： ",fp);
		sprintf(chr,"%d",plist.front().wait_time);
		fputs(chr,fp);
		fputs("\n当前优先级： ",fp);
		sprintf(chr,"%f",plist.front().priority);
		fputs(chr,fp);
		fclose(fp);
	}
	else
...
/*
	功能：
		优先级调度算法的实现，改变等待状态的PCB和阻塞状态的PCB的优先级
	参数：
		ite：运行/等待队列和阻塞的迭代器，用来进行遍历
		plist：运行/等待队列
		pri_list：阻塞队列
		is_run：运行状态，1为运行，0位等待
		wait_time：PCB已等待时间
		priority：PCB优先级
...
void HRN(){
	...
	for(ite = plist.begin();ite!=plist.end();ite++){
		//遍历运行/等待队列，将每个等待状态的元素的等待时间+1，改变其优先级
		if((*ite).is_run == 0){
			(*ite).wait_time++;
...
void *fork_progress1(void *arg){
	...
	while(1){
		...
		if(plist.empty()){
			if(pri_list.empty()){		//如果运行/等待队列和阻塞队列均为空，则表示所有进程都运行完毕
				change_stdout("3");
				cout<<"\nover!\n";
				close_stdout();
				break ;
			}
			else{						//如果阻塞队列不为空，则自动将阻塞队列的最大优先级的元素移动到运行/等待队列
...
int main(int argc,char *argv[]){
	...
	while(1){
		...
		{	
				...
				for (i = 0; i < num; ++i)
				{
					...
					if(i == 0)
						...
						PCB.is_run = 0;					//否则先等待，需判断优先级
					plist.push_back(PCB);				//将创建的PCB插入到运行/等待队列
					//在编号为3的终端打印该PCB的信息
					change_stdout("3");
		...
		//输入wake n：将id为n的进程PCB唤醒，即从阻塞队列移动到运行/等待队列
		else if(in[0] == 'w'){
			inn = in.c_str();
			int num;
		...
		//输入exit n：撤销id为n的进程PCB
			...
			pthread_mutex_lock(&lock);
			Erase_plist(num);						//从运行/等待队列中将该PCB移除
			change_stdout("1");
			printf(" %d exit\n",num );				//打印提示信息
			close_stdout();				
			pthread_mutex_unlock(&lock);
